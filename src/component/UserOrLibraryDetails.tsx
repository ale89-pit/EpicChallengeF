...
const [userDto, setUserDto] = useState<userDto>({
  ...
  const uriModify = currentProfile.roles[0].roleName === "ROLE_USER" ?"http://localhost:8080/user/":"http://localhost:8080/library/";
  ...
  const getMunicipality = async (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
        let response = await fetch(
            `http://localhost:8080/municipality/${selectedProvince}`, requestOptions
        )
        if(response.ok){
            let data = await response.json();
            console.log(data);
      ...
      <Form noValidate validated={validated} onSubmit={handleSubmit}>
        <Row className="mb-3">
         ...
         <Form.Group as={Col} md="4" controlId="validationFullName" >
          ...
          {currentProfile.username &&<Form.Group as={Col} md="4" controlId="validationUsername" onChange={handleInputChange}>
          ...
          {currentProfile.name &&<Form.Group as={Col} md="4" controlId="validationUsername" onChange={handleInputChange}>
          ...
          <Form.Group as={Col} md="4" controlId="validationPhone">
            ...
            <InputGroup.Text id="inputGroupPrepend">☎️</InputGroup.Text>
              <Form.Control
                ...
                required
                aria-describedby="inputGroupPrepend"
                defaultValue={currentProfile.phone} onChange={(e)=>setUserDto({...userDto,phone:e.currentTarget.value})}    
                name="phone"
              />
            </InputGroup>
          </Form.Group>
        ...
        <Row className="mb-3">
          <Form.Group as={Col} md="6" controlId="validationRegione">
            ...
            <Form.Select aria-label="Default select example" onChange={filterProvince}>
                <option>{currentProfile.address!= null?currentProfile.address.municipality.province.region:""}</option>
              {region.map((element) => {
          ...
          <Form.Group as={Col} md="3" controlId="validationComune">
            ...
            <Form.Select aria-label="Default select example" onChange={(e)=>setUserDto({...userDto,addressDto:{...userDto.addressDto,municipalityId:e.currentTarget.value}})}>
            <option>{currentProfile.address!= null?currentProfile.address.municipality.name:""}</option>
             {municipality.map((element) => {
          ...
          <Form.Group as={Col} md="3" controlId="validationStreet">
            ...
            <Form.Control type="text" placeholder="Via Roma" required 
            defaultValue={currentProfile.address!= null?currentProfile.address.street:""}
            onChange={(e)=>setUserDto({...userDto,addressDto:{...userDto.addressDto,street:e.currentTarget.value}})}/>
          ...
          <Form.Group as={Col} md="3" controlId="validationStreetNumber">
            ...
            <Form.Control type="string" placeholder="56" 
            defaultValue={currentProfile.address!= null?currentProfile.address.number:""}
            onChange={(e)=>setUserDto({...userDto,addressDto:{...userDto.addressDto,streetNumber:e.currentTarget.value}})} />
            <Form.Control.Feedback type="invalid">
